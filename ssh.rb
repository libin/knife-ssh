    ...
    class Ssh < Knife
      ...
      def print_data(host, data)
        if data =~ /\n/
          ...
          unless host_color = HOSTS[host]
            HOSTS[host] = host_color = (host_colors = Chef::Config[:knife][:colors]) ? host_colors[HOSTS.size%host_colors.size] : Paint.random
          end
          padding = @longest - host.length
          str = if Chef::Config[:knife][:color_content]
            Paint[(host + (" " * (padding + 1)) + data), host_color]
          else
            Paint[host, host_color] + (" " * (padding + 1)) + data
          end
          ui.msg str
        end
      end
